...
*(EN: This project involves modeling a relational database designed to support the complete sales cycle of a supermarket, with a strategic focus on **transaction traceability** and **customer loyalty**.)*
...
*(EN: The solution aims to transform transactional data into analytical inputs ready for the customer loyalty program and marketing actions.)*

## 💡 O Diagnóstico do Problema | The Problem Diagnosis
...
*(EN: The initial diagnosis identified the need to structure the database to solve two main problems: 1) **Lack of Traceability**: Inconsistency in sales records hindering tax auditing and inventory data accuracy. 2) **Inability to Perform Loyalty Analysis**: No clear connection existed between sales and registered customers, preventing point calculation and segmentation for loyalty campaigns.)*
...
### Mapeamento Detalhado: Perguntas de Negócio para Solução Técnica | Detailed Mapping: Business Questions to Technical Solution
...
> *(EN: What minimum data is required to validate and register a transaction in the system, even if the customer is not identified?)*
...
> *(EN: How does the system handle sales containing multiple products? And what if the price changes?)*
...
> *(EN: Is it possible for the customer to split the payment amount using, for example, Pix and Card in the same transaction?)*
...
> *(EN: Customer Identification: What is the unique identifier field for the customer? Can they have more than one phone number or address?)*
...
> *(EN: Can an employee work at more than one store unit at the same time, or are they fixed to only one?)*
...
> *(EN: Supplier Identification: What is the main and unique identifier for the supplier?)*
...
> *(EN: Do stores and suppliers follow the same rule as customers, being able to have multiple registered addresses?)*
...
> *(EN: What is the main tracking code for the product? What other inventory data is critical?)*
...
> *(EN: What is the relationship type between a Customer and a Loyalty Program? What data do we need to know about this enrollment?)*
...
> *(EN: Can a product be bought from different suppliers? And are the cost and delivery time always the same?)*
...
## 🛠️ Decisões Chave de Modelagem | Key Modeling Decisions
...
*(EN: The logical model was built prioritizing data integrity and performance in BI (Business Intelligence) queries.)*

| Tópico | Decisão Implementada | Justificativa |
| :--- | :--- | :--- |
| **Identificadores (CPF/CNPJ)** | Uso do tipo de dado **`CHAR(11)`** para CPF e **`CHAR(14)`** para CNPJ. | Garante tamanho fixo para otimizar o desempenho em buscas e índices, padronizando a entrada de dados. (EN: Ensures fixed length for optimized search and indexing performance, standardizing data entry.) |
| **Valores Monetários** | Uso do tipo de dado **`DECIMAL(10,2)`** (e não `FLOAT`). | Essencial para garantir a **precisão fiscal** e evitar erros de arredondamento inerentes ao ponto flutuante, crucial para transações financeiras. (EN: Essential for ensuring **fiscal precision** and avoiding rounding errors inherent in floating point, crucial for financial transactions.) |
| **Auditoria e Tempo** | Campo `venda_data_hora` com **`DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP`**. | Assegura que o registro de data e hora seja feito de forma automática pelo SGBD no momento da inserção, garantindo a rastreabilidade e integridade temporal da venda. (EN: Ensures the date and time record is automatically made by the DBMS upon insertion, guaranteeing traceability and temporal integrity of the sale.) |
| **Relacionamento M:N** | Criação da tabela **`tbl_item_venda`** com uma **Chave Primária Composta** (`id_venda`, `id_produto`). | Resolve a relação N:N (uma Venda tem muitos Produtos; um Produto está em muitas Vendas), permitindo rastrear o preço e a quantidade de cada item na nota fiscal. (EN: Resolves the N:N relationship, allowing the tracking of the price and quantity of each item on the invoice.) |

## 📂 Estrutura das Tabelas Principais | Main Table Structure
...
## 🚀 Valor para o Negócio (Fidelização) | Business Value (Loyalty)

A arquitetura do banco de dados permite que a equipe de marketing realize consultas estratégicas, como:

*(EN: The database architecture allows the marketing team to perform strategic queries, such as:)*
...
* [`ciclo_vendas_supermercado.sql`](ciclo_vendas_supermercado.sql): Script completo para criação do banco de dados. (EN: Complete script for database creation.)
* [`modelo_conceitual-supermercado1.png`](modelo_conceitual-supermercado1.png): Diagrama do Modelo Conceitual. (EN: Conceptual Model Diagram.)
* [`modelo_logico_ciclo_vendas_supermercado.png`](modelo_logico_ciclo_vendas_supermercado.png): Diagrama do Modelo Lógico com chaves e tipos de dados. (EN: Logical Model Diagram with keys and data types.)
